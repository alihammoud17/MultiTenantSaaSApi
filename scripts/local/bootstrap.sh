...
printf '[local-bootstrap] Restoring .NET dependencies...\n'
dotnet restore

printf '[local-bootstrap] Building .NET solution...\n'
dotnet build --no-restore

if [[ -x "$DOTNET_EF_BIN" ]]; then
  printf '[local-bootstrap] Applying EF migrations with %s...\n' "$DOTNET_EF_BIN"
  "$DOTNET_EF_BIN" database update --project Infrastructure --startup-project Presentation
else
  printf '[local-bootstrap] WARNING: %s not found; skipping migration apply.\n' "$DOTNET_EF_BIN"
  printf '[local-bootstrap] Manual step: dotnet ef database update --project Infrastructure --startup-project Presentation\n'
fi

printf '[local-bootstrap] Installing BillingService dependencies (npm ci)...\n'
(
  cd BillingService
  npm ci
)
...
printf 'Next steps:\n'
printf '  1) scripts/local/run.sh\n'
printf '  2) scripts/local/smoke.sh\n\n'
